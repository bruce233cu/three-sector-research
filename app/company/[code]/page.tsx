...
export default function Page() {
  ...
  const sections = [
    ...
    ["06","关键缺口",c.metadata?.gaps || c.keyAssumptions],
  ];
  return (
    <Shell active="公司研究">
      ...
      <div className="record-head">
        <div>
          ...
          <p>最后更新：{c.metadata?.profile_date || c.updatedAt?.slice(0,10) || "待确认"} · 核心假设验证中</p>
        </div>
        <div>
          <span>综合置信度</span>
          <strong>
      ...
      <div className="decision-strip">
        ...
        <div><span>核心催化</span><strong>{c.reason}</strong></div>
        <div><span>最大缺口</span><strong>{c.metadata?.gaps || "利润映射尚未完成"}</strong></div>
      </div>
